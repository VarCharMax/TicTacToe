    ...
    public class CommunicationMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            if (context.WebSockets.IsWebSocketRequest)
            {
                ...
                {
                    _diagnosticSource.Write("TicTacToe.MiddlewareStarting", new { httpContext = context });
                }
        ...
        private async Task ProcessGameInvitationConfirmation(HttpContext context, WebSocket webSocket, CancellationToken ct, string parameters)
        {
            ...
            {
                await SendStringAsync(webSocket, "WaitGameConfirmation", ct);
            ...
            if (gameInvitationModel.IsConfirmed == true)
            {
        ...
        private async Task ProcessGameInvitationConfirmation(HttpContext context)
        {
            ...
            if (string.IsNullOrEmpty(id)) await context.Response.WriteAsync("Bad Request: Id is required");
        ...
        private async Task ProcessEmailConfirmation(HttpContext context, WebSocket currentSocket, CancellationToken ct, string email)
        {
            ...
            while (!ct.IsCancellationRequested && !currentSocket.CloseStatus.HasValue && user?.EmailConfirmed == false)
            {
        ...
        private async Task ProcessEmailConfirmation(HttpContext context)
        {
            ...
            {
                await context.Response.WriteAsync("Bad Request: Email is required");
            }
            else if ((await userService.GetUserByEmail(email)).EmailConfirmed)
            {
                await context.Response.WriteAsync("OK");
            }   
            else
            {
                await context.Response.WriteAsync("Waiting For Email Confirmation");
            }
        }
